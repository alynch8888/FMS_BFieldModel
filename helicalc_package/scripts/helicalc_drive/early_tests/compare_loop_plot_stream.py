...
def make_stream(df, x='Z', y='X', vec=['Bz_calc','Bx_calc'], title='Helicalc', savename='helicalc_coarse'):
# def make_stream(df, x='Z', y='X', vec=['Bz','Bx']):
    ...
    V = 1e4*(VX**2 + VY**2)**(1/2)
    print(X.shape, Y.shape, VX.shape, VY.shape, V.shape, lx, ly)
    fig = plt.figure()
    ax = fig.add_subplot(111)
    cp = ax.contourf(X, Y, V, levels=100)
    cb = plt.colorbar(cp)
    ...
    qp = ax.quiver(df_[x], df_[y], df_[vec[0]], df_[vec[1]], color=(0.7,0.7,0.7), zorder=25, pivot='mid', scale=1, width=0.004)
    # cb = plt.colorbar(qp)
    # cb.set_label(r"$|B|_{\mathrm{Z,X plane}}$ [Gauss]")

    plt.legend(handles=[rp], loc='upper right')
    # plt.xlim([-.0021, .0021])
    # plt.xlim([-.0051, .0051])
    # plt.ylim([1.055, 1.067])
    plt.xlim([df_[x].min()-.01, df_[x].max()+.01])
    plt.ylim([df_[y].min()-.01, df_[y].max()+.01])
    plt.xlabel(x+' [m]')
    plt.ylabel(y+' [m]')
    plt.title(title+' Near Conductor, Y==0 [m]')
    ax.set_aspect('equal')
    # plt.set_aspect('equal')
    # fig.savefig(plotdir+'stream_plot_conductor_02.pdf')
    # fig.savefig(plotdir+'stream_plot_conductor_02.png')
    fig.savefig(plotdir+f'quiver_plot_conductor_{savename}.pdf')
    fig.savefig(plotdir+f'quiver_plot_conductor_{savename}.png')

make_stream(df)
make_stream(df, vec=['Bz_br','Bx_br'], title='Opera Brick (BR20)', savename='opera_brick')
make_stream(df, vec=['Bz','Bx'], title='Opera Solenoid (GSOLENOID)', savename='opera_solenoid')
