...
# standard
# plotdir = helicalc_data+'plots/helicalc/validation/DS8/'
# radius optimize
plotdir = helicalc_data+'plots/helicalc/validation/DS8/optimize/'

# standard
# data_file = helicalc_data+'Bmaps/helicalc_validation/Mu2e_V13.DS8_region_plane.standard-helicalc.coil_63_full.pkl'
# radius optimize
deltaR=-2.313e-3
...
for y in [0.0]:
    for i in ['x', 'y', 'z']:
        ...
        ax2.set_title('OPERA\n(Helical Coil + Interlayer Connect)')
        fig.savefig(plotdir+f'DS8_B{i}_vs_X_Z_Y_{y:0.2f}_compare.pdf')
        fig.savefig(plotdir+f'DS8_B{i}_vs_X_Z_Y_{y:0.2f}_compare.png')
...
for y in [0.0]:
    for i in ['x', 'y', 'z']:
        ...
        fig.suptitle(f'(B{i}_helicalc - B{i}_OPERA) vs X and Z for DS-8\n-0.9<=X<=0.9, Y=={y}, 7.621<=Z<=9.946')
        ax.set_title(None)
        #         ax.set_xlabel('R (m)')
#         ax.set_ylabel('Z (m)')
        fig.savefig(plotdir+f'DS8_deltaB{i}_vs_X_Z_Y_{y:0.2f}_compare.pdf')
        fig.savefig(plotdir+f'DS8_deltaB{i}_vs_X_Z_Y_{y:0.2f}_compare.png')
