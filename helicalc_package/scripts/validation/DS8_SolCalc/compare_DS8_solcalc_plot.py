...
# standard
plotdir = helicalc_data+'plots/helicalc/validation/SolCalc/DS8/'
# radius optimize
...
for y in [0.0]:
    for i in ['x', 'y', 'z']:
        ...
        fig = mu2e_plot3d(df, 'X', 'Z', f'B{i}_solcalc_63', f'Y=={y}', units='m', mode='mpl', fig=fig, ax=ax2)
        fig.suptitle(f'B{i} vs X and Z for DS-8\n-0.9<=X<=0.9, Y=={y}, 7.621<=Z<=9.946')
        ax1.set_xlabel('R (m)')
        ax1.set_ylabel('Z (m)')
        ax1.set_title('Helicalc\n(Helical Coil)')
        ax2.set_title('SolCalc\n(Ideal Coil)')
...
for y in [0.0]:
    for i in ['x', 'y', 'z']:
        ...
        fig.suptitle(f'(B{i}_helicalc - B{i}_SolCalc) vs X and Z for DS-8\n-0.9<=X<=0.9, Y=={y}, 7.621<=Z<=9.946')
        ax.set_title(None)
        #         ax.set_xlabel('R (m)')
#         ax.set_ylabel('Z (m)')
