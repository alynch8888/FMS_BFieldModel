...
def brzphi_3d_producer_giant_function_v1001(z, r, phi,
                                            pitch1, ms_h1, ns_h1,
                                            pitch2, ms_h2, ns_h2,
                                            length1, ms_c1, ns_c1,
                                            length2, ms_c2, ns_c2):
    ...
    @njit(parallel=True)
    def calc_b_fields_cyl(z, phi, r, cms, n, A, C, D, iv, ivp, model_r, model_z, model_phi):
        for i in prange(z.shape[0]):
            if n > 0:
                model_r[i] += C * (D*np.sin(n*phi[i]) + (1-D)*np.cos(n*phi[i])) * \
                    ivp[i]*cms*(A*np.cos(cms*z[i]) + (1-A)*np.sin(cms*z[i]))

                model_z[i] += C * (D*np.sin(n*phi[i]) + (1-D)*np.cos(n*phi[i])) * \
                    iv[i]*cms*(-A*np.sin(cms*z[i]) + (1-A)*np.cos(cms*z[i]))
                ...
                #if not np.isclose(r[i], 0):
                if abs(r[i]) >= 1e-5:
                    model_phi[i] += C * n*(D*np.cos(n*phi[i]) - (1-D)*np.sin(n*phi[i])) * \
                        (1/r[i])*iv[i]*(A*np.cos(cms*z[i]) + (1-A)*np.sin(cms*z[i]))
            else:
                model_r[i] += C*(1-D) * \
                    ivp[i]*cms*(A*np.cos(cms*z[i]) + (1-A)*np.sin(cms*z[i]))
    ...
    def brzphi_3d_fast(z, r, phi, x, y, **AB_params):
        ...
        for m in range(ms_c1):
            for n in range(ns_c1):
                ...
                D = AB_params[f'Dc1_{n}']
                calc_b_fields_cyl(z, phi, r, cms1[m], n, A, C, D, iv_c1[m][n], ivp_c1[m][n],
                                  model_r, model_z, model_phi)
...
def brzphi_3d_producer_giant_function_v1002(z, r, phi,
                                            pitch1, ms_h1, ns_h1,
                                            pitch2, ms_h2, ns_h2,
                                            length1, ms_c1, ns_c1,
                                            length2, ms_c2, ns_c2):
    ...
    @njit(parallel=True)
    def calc_b_fields_cyl(z, phi, r, cms, n, A, B, C, D, iv, ivp, model_r, model_z, model_phi):
        for i in prange(z.shape[0]):
            if n > 0:
                model_r[i] += (D*np.sin(n*phi[i]) + C*np.cos(n*phi[i])) * \
                    ivp[i]*cms*(A*np.cos(cms*z[i]) + B*np.sin(cms*z[i]))

                model_z[i] += (D*np.sin(n*phi[i]) + C*np.cos(n*phi[i])) * \
                    iv[i]*cms*(-A*np.sin(cms*z[i]) + B*np.cos(cms*z[i]))
                ...
                #if not np.isclose(r[i], 0):
                if abs(r[i]) >= 1e-5:
                    model_phi[i] += n*(D*np.cos(n*phi[i]) - C*np.sin(n*phi[i])) * \
                        (1/r[i])*iv[i]*(A*np.cos(cms*z[i]) + B*np.sin(cms*z[i]))
            else:
                model_r[i] += C * \
                    ivp[i]*cms*(A*np.cos(cms*z[i]) + B*np.sin(cms*z[i]))
    ...
    def brzphi_3d_fast(z, r, phi, x, y, **AB_params):
        ...
        for m in range(ms_c1):
            for n in range(ns_c1):
                A = AB_params[f'Ac1_{m}_{n}']
                B = AB_params[f'Bc1_{m}_{n}']
                C = AB_params[f'Cc1_{n}']
                D = AB_params[f'Dc1_{n}']
                calc_b_fields_cyl(z, phi, r, cms1[m], n, A, B, C, D, iv_c1[m][n], ivp_c1[m][n],
                                  model_r, model_z, model_phi)
...
# trying to use Y_n along with J_n
# NOT DONE YET!!!
def brzphi_3d_producer_giant_function_v1003(z, r, phi,
                                            pitch1, ms_h1, ns_h1,
                                            pitch2, ms_h2, ns_h2,
                                            length1, ms_c1, ns_c1,
                                            length2, ms_c2, ns_c2):
...
def brzphi_3d_producer_giant_function_v1004(z, r, phi,
                                            pitch1, ms_h1, ns_h1,
                                            pitch2, ms_h2, ns_h2,
                                            length1, ms_c1, ns_c1,
                                            length2, ms_c2, ns_c2,
    ...
    def calc_b_fields_cyl(z, phi, r, cms, n, A, B, D, iv, ivp, model_r, model_z, model_phi):
        for i in prange(z.shape[0]):
            if n > 0:
                ...
                if abs(r[i]) >= 1e-5:
                    model_phi[i] += n*A*(np.cos(n*phi[i]+D))*(1/r[i])*iv[i]*(np.sin(cms*z[i]+B))
            else:
...
def brzphi_3d_producer_giant_function_v1005(z, r, phi,
                                            pitch1, ms_h1, ns_h1,
                                            pitch2, ms_h2, ns_h2,
                                            length1, ms_c1, ns_c1,
                                            length2, ms_c2, ns_c2,
